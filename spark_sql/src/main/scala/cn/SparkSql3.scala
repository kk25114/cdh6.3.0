...
object SparkSql3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //细目表
    val itemDF = spark.read.jdbc("jdbc:mysql://127.0.0.1:3306", "test.dw_dim_item", connectionProperties)
    itemDF.createTempView("dw_dim_item")

    //科目表
    val subjectDF = spark.read.jdbc("jdbc:mysql://127.0.0.1:3306", "test.kn_subject", connectionProperties)
    subjectDF.createTempView("kn_subject")

    //成绩表DF
    var cjDF = spark.read.jdbc("jdbc:mysql://127.0.0.1:3306", "test.dw_km_cj_details", connectionProperties)
    ...
    subjectids.foreach(subjectId => {
      ...
      df.show()
      //保存结果
      df.write.mode(SaveMode.Overwrite).jdbc("jdbc:mysql://127.0.0.1:3306", "test.dw_subject_result3", connectionProperties)
      ...
      highDf.show()
      //保存结果
      highDf.write.mode(SaveMode.Overwrite).jdbc("jdbc:mysql://127.0.0.1:3306", "test.dw_subject_result4", connectionProperties)
    })
