...
object SparkSql3_bak {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def calItemDD(x: Row, subjectId: Long): Double = {
      ...
      //高分组平均分
      val avg1 = spark.sql("SELECT AVG(" + column + ") FROM dw_km_cj_details WHERE examid=50 AND subjectid=" + subjectId + " AND zcj>=" + highScore).collect()(0).getDouble(0);
      //低分组平均分
      val avg2 = spark.sql("SELECT AVG(" + column + ") FROM dw_km_cj_details WHERE examid=50 AND subjectid=" + subjectId + " AND zcj<=" + lowScore).collect()(0).getDouble(0);
      //返回  小题区分度
      (avg1 - avg2) * 1.0 / fullScore
    }

    //细目表
    val itemDF = spark.read.jdbc("jdbc:mysql://10.214.22.46:3306", "test_gkpj.dw_dim_item", connectionProperties)
    itemDF.createTempView("dw_dim_item")

    //科目表
      val subjectDF = spark.read.jdbc("jdbc:mysql://10.214.22.46:3306", "test_gkpj.kn_subject", connectionProperties)
    subjectDF.createTempView("kn_subject")

    //成绩表DF
    var cjDF = spark.read.jdbc("jdbc:mysql://10.214.22.46:3306", "test_gkpj.dw_km_cj_details", connectionProperties)
    ...
    subjectids.foreach(subjectId => {
      ...
      df.show()
      //保存结果
//      df.write.mode(SaveMode.Overwrite).jdbc("jdbc:mysql://10.214.22.46:3306", "test.dw_subject_result3", connectionProperties)
      ...
      highDf.show()
      //保存结果
//      highDf.write.mode(SaveMode.Overwrite).jdbc("jdbc:mysql://10.214.22.46:3306", "test.dw_subject_result4", connectionProperties)
    })
